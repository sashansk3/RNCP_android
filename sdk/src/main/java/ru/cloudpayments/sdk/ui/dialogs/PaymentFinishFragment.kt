...
internal class PaymentFinishFragmentFragment: BasePaymentDialogFragment<PaymentFinishViewState, PaymentFinishViewModel>() {
	...
	companion object {
		...
		fun newInstance(status: PaymentFinishStatus, transaction: CloudpaymentsTransaction, reasonCode: String, retryPayment: Boolean) = PaymentFinishFragmentFragment().apply {
			arguments = Bundle()
			arguments?.putString(ARG_STATUS, status.toString())
	...
	private val transaction by lazy {
		when {
			android.os.Build.VERSION.SDK_INT >= 33 -> arguments?.getParcelable(ARG_TRANSACTION, CloudpaymentsTransaction::class.java) ?: null
